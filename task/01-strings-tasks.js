...
function getRectangleString(width, height) {
    return `${height - 2 >= 0 ? `${`┌${width - 2 > 0 ? '─'.repeat(width - 2) : ''}┐\n`}${`│${width - 2 > 0 ? ' '.repeat(width - 2) : ''}│\n`.repeat(height - 2)}${`└${width - 2 > 0 ? '─'.repeat(width - 2) : ''}┘\n`}` : ''}`;
}
...
function encodeToRot13(str) {
    return str.replace(/[a-zA-Z]/g, function (match) { return String.fromCharCode((match <= "Z" ? 90 : 122) >= (match = match.charCodeAt(0) + 13) ? match : match - 26); });
}
        ...
        if (value[0] == 'J')
            ...
            if (value[0] == 'Q')
                ...
                if (value[0] == 'K')
                    ...
                    return +value.slice(0, -1) + result - 1;
}
